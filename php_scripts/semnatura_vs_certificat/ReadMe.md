...
Pentru **API**-uri scrise in PhP sunt mai multe [***exemple-de-cod***](https://lorand.work/autentificare-oauth-si-obtinere-token-jwt-de-la-anaf-folosind-php/) pe piata in ceea ce priveste procedura ***OAuth*** bazata pe **JWT**. Desigur, accentul discutiei noastre, se pune doar, pe [*generarea-**JWT***](https://static.anaf.ro/static/10/Anaf/Informatii_R/API/Oauth_procedura_inregistrare_aplicatii_portal_ANAF.pdf) sau mai bine spus *administrarea*/*gestionarea* token-ului [**JWT**](https://iapp.ro/articol/generare-token-anaf-folosind-certificat-digital-din-php-oauth2), deoarece discutia despre scopul API-ului, in sine(deci care se refera la utilizarea efectiva a  serviciilor-web, expuse de catre ANAF), este  mult mai elaborata si a fost discutata, pe larg, in alte posturi conexe, specializate in acest sens. Ca si conditii prealabile, trebuie sa aveti, *semnatura digitala* si *certificatul calificat* precum si inregistrarea *utilizator*ului/*dezvoltator*ului impreuna cu *inregistrarea aplicatiei*/*website*-ului/*API*-ului dvs(ca si PJ/firma/companie..., spre ex.: PFA, SRL...),  in SPV(deja procurate/efectuate), inainte de a genera *token*-ul **JWT** de access, ca mai apoi, in cele din urma, sa puteti sa accesati serviiciile online/web ale **ANAF**-ului(adica, sa accesati serciiile ***eFactura***/***eTransport***... ale **ANAF**-ului... din **API**-ul dvs, dezvoltat de dezvoltatorul dvs, ca si PJ, atunci cand utilizatorii dvs, il folosesc!) . Recomand acest exemplu [JWT](https://github.com/firebase/php-jwt) firebase ca si [sablon/model](https://stackoverflow.com/questions/41795034/class-firebase-jwt-jwt-not-found) de [lucru/dezvoltare](https://www.printcenter.ro/module/gapi/vendor/firebase/php-jwt/src/). In cazul in care doriti sa lucrari cu WordPress(WP) acest [post](https://ro.wordpress.org/plugins/jwt-authentication-for-wp-rest-api/) va poate fi de ajutor.
