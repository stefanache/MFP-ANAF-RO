...
<?php
	set_time_limit(-1);
	//	http://localhost/MFP/importXML_PCs_StrOthers.php
	...
	$table_name="nom_cod_postale_altele_strazi";
	...
	for ($cod_judLoc = 1; $cod_judLoc < 41; $cod_judLoc++ ){
		...
		//$cod_judLoc=1;				 //ALBA
		// get xml from url
		//$url = 'https://mfinante.gov.ro/static/40/Mfp/nomenclatoare/nomCodAlteleStazi_1_02.02.2024.xml';		
		
		$url="https://mfinante.gov.ro/static/40/Mfp/nomenclatoare/nomCodAlteleStazi_".$cod_judLoc."_".$data_publicare.".xml";
		...
		if($have_data){
			foreach ($xml->rand as $randElement) {
				...
				// start sql statement
				$sql_insert = "INSERT INTO ".$table_name."(pk_cpaltestr, LOC_JUD_COD, LOC_COD, COD, BLOC, COD_POSTAL )
				VALUES ($row_crt, $loc_jud_cod, $loc_cod, $cod, '$bloc', '$cod_postal' )";
